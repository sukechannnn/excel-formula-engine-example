...
describe("FormulaToRPNConverter", () => {
  ...
  describe("基本的な数式", () => {
    it("数値のみ", () => {
      ...
      expect(result.tokens).toEqual([{ type: TokenType.NUMBER, value: 42 }])
      expect(result.dependencies).toEqual([])
    })

    it("セル参照のみ", () => {
      const result = parseAndConvert("=A1")
      expect(result.tokens).toEqual([{ type: TokenType.CELL_REF, value: "A1" }])
  ...
  describe("SUM関数", () => {
    ...
    it("引数なし", () => {
      ...
      expect(result.tokens).toEqual([{ type: TokenType.NUMBER, value: 0 }])
    })
  ...
  describe("IF関数", () => {
    ...
    it("ネストしたIF", () => {
      ...
      const hasNestedJumps = result.tokens.filter(
        (t) => t.type === TokenType.JUMP_IF_FALSE || t.type === TokenType.JUMP
      ).length
  ...
  describe("単項演算子", () => {
    ...
    it("単項プラス", () => {
      const result = parseAndConvert("=+5")
      expect(result.tokens).toEqual([{ type: TokenType.NUMBER, value: 5 }])
    })
